...
class NextReactionMethod(object):
    ...
    def PR_PRM_model_config_update1(self, copy, val, system_time):
    ...
    def PRE_model_config_update2(self, copy, val, system_time):
    ...
    def PL_model_config_update3(self, copy, val, system_time):
    ...
    def promoter_occupancy_states(self, system_time):
        ...
        while np.sum(switch.values()) > 0:
            ...
            for i in F:
                ...
                if len(Y) > 0:
                    ...
                    val = Y[0]
                    X = V[i][1](val)
                    V[i][2](val,X,system_time)
                    for j in range(len(Y)):
                        if j != 0:
                            temp.append(Y[j])
                    switch[V[i][0]] = temp

    def reverse_initiation_constant(self, system_time):
    ...
    def engage_isomer_reactions(self, system_time, reaction_object):
        R = reaction_object
    ...
    def form_queue_structure(self,i):
        L = self.total_reactions
        #i is the index of the list
        even = (2*i + 2) #the even 'daughter' node
        odd = (2*i + 1) #the odd 'daughter' node
        if not i: #case of zero index
            if L[i].tau > L[odd].tau: #swap function, if parent node value greater, swap with daughter
                ref = L[i]
                L[i] = L[odd]
                L[odd] = ref
            if L[i].tau > L[even].tau:
                ref = L[i]
                L[i] = L[even]
                L[even] = ref
        else: # 
            if even < len(L):   
                if L[i].tau > L[odd].tau:
                    ref = L[i]
                    L[i] = L[odd]
                    L[odd] = ref
                if L[i].tau > L[even].tau:
                ...
                if L[i].tau > L[odd].tau:
                    ...
                    L[i] = L[odd]
                    L[odd] = ref
            if (i/2)*2 == i: #checking to see if i is even
                back = (i-2)/2 #regress back to the parent node
            else:
                back = (i-1)/2
            self.form_queue_structure(back) #call the function again

    def initialize_priority_queue(self): #proceeds to move through each index and find and do the recursive function
        L = self.total_reactions
        n = 0
        while (2*n + 1) < len(L):
            self.form_queue_structure(n)
            n += 1

    def initialize_locus_dict(self):
        locus_dict = {}
        for i in range(len(self.total_reactions)):
            locus_dict.update({self.total_reactions[i]:i})

        self.locus = locus_dict

    def recursive_up(self,i):
        L = self.total_reactions
        odd = 2*i + 1
        even = 2*i + 2
        if not i:
            if L[i].tau > L[odd].tau:
                ...
                if L[i].tau > L[even].tau:
                    ...
                    L[i] = ref
                    self.locus[L[i]] = i
                    self.locus[L[even]] = even
                    self.recursive_down(even)
            
            elif L[i].tau > L[even].tau:
                ref = L[even]
                L[even] = L[i]
                L[i] = ref
                self.locus[L[i]] = i
        ...
        if odd < len(L):
            if even < len(L):
                if L[i].tau > L[odd].tau:
                    ...
                    if hit:    
                        self.recursive_down(even)
                        self.recursive_down(odd) #lets see if this works!
                    else:
                        self.recursive_down(odd)
    ...
    def gauge_direction(self, rxn_obj):
        L = self.total_reactions
        index = self.locus[rxn_obj]
        odd = 2*index + 1
        even = 2*index + 2
        if index:
            if (index/2)*2 == index: 
        ...
        if odd < len(L):
            if even < len(L):
                if L[index].tau > L[odd].tau:
                    self.recursive_down(index)
                elif L[index].tau > L[even].tau:
                    self.recursive_down(index)
                else:
                    if index:
                        self.recursive_up(back)
    ...
    def promoter_decoupling(self, reaction_index, system_time):
        ...
        self.reverse_initiation_constant(system_time)
    ...
    def NRM_execution(self, step, leap, end, DNA_init, reaction_constants):
        ...
        for i in range(step):
            ...
            if i > 0:
                ...
                self.promoter_decoupling(reaction_index, system_time)

            self.promoter_occupancy_states(system_time)

            reaction_obj = self.total_reactions[0]

            reaction_index = 0

            dependency_list = self.dep_graph[reaction_obj]

            system_time = reaction_obj.tau

            if reaction_obj.reactants == reaction_obj.products:
                for j in reaction_obj.reactants:
                    new = j.count - 1
                    j.count = new
                ...
                for j in reaction_obj.reactants:
                    new = j.count - 1
                    j.count = new
                for j in reaction_obj.products:
                    new = j.count + 1
                    j.count = new
            ...
            for j in dependency_list:
                if j == reaction_obj:       
                    ...
                    self.gauge_direction(j)

                else:
                    Q = [val for val in self.isomer_react_dict.keys() if val == j]
                    
                    if len(Q) == 1:
                        self.engage_isomer_reactions(system_time, j)
                    else:
                        j.get_propensity()
            ...
            self.tau_list.append(system_time)