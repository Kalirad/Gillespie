...
class RNA(Species):
    def __init__(self, name, tag_rna, rna_length, promoter_affiliation, time, pre_termination=False, absolute=False, initial=False, prot=False, count=False):
        ...
        absolute : int
                The absolute number of molecules synthesized

        initial : int
                The count of the pool at the beginning of the allocated pool time

        prot : int
                The number of proteins synthesized during that pool interval
    ...
    def create_rna_rate_dict(self):
        self.rna_rate_dict = {'rna_synth':[], 'rna_deg':[], 'prot_synth':[]}


    def calculate_rate_parameters(self):
        ref = self.absolute
        ini = self.initial
        rna_deg = (ref + ini) - self.count
        rna_synth = ref
        prot_synth = self.prot
        for i in self.rna_rate_dict.keys():
            X = self.rna_rate_dict[i]
            if i == 'rna_synth':
                X.append(rna_synth)
            elif i == 'rna_deg':
                X.append(rna_deg)
    ...
    def reset_pool_attributes(self, time_leap):
        k = time_leap
        self.absolute = False
        self.prot = False
    ...
    def change_pool_attributes(self, system_time, time_leap):
        if system_time => i.time:
            self.calculate_rate_parameters()
            self.reset_pool_attributes(time_leap)
...
class NextReactionMethod(object):
    ...
    def initialize_rna_spec_rates(self):
        for i in self.species:
            if type(i) == RNA:
                i.create_rna_rate_dict()
    ...
    def NRM_execution(self, step, leap, pool_leap, end, DNA_init, reaction_constants):
        ...
        for i in range(step):
            ...
            for j in self.species:
                if type(j) == RNA:
                    j.change_pool_attributes(system_time, pool_leap)
